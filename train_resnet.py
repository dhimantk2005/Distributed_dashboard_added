"""
Distributed ML Training across laptops via Tailscale (ResNet)
============================================================
Usage (run on EVERY machine):

  Master node (rank 0):
    python train_resnet.py --rank 0 --world-size 2 --master-addr <your-tailscale-ip>

  Worker node (rank 1):
    python train_resnet.py --rank 1 --world-size 2 --master-addr <master-tailscale-ip>
...
def parse_args():
    p = argparse.ArgumentParser(description="Distributed CIFAR-10 training (ResNet)")
    ...
    for attempt in range(1, attempts + 1):
        ...
        try:
            ...
            if "Address already in use" in err or "Address in use" in err:
                ...
                print(f"         - Is master running? (python train_resnet.py --rank 0 ...)")
